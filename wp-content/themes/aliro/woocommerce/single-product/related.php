 ...
 * @version     3.0.0
 */
if( !(shu_fnc_theme_options('wc_show_related', false)) ){
	if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

		global $product, $woocommerce_loop;
		$posts_per_page = shu_fnc_theme_options('woo-number-product-single',6);
		...
		$columns_count = shu_fnc_theme_options('product-number-columns',3);
		$class_column = 'col-md-' . floor( 12/$columns_count );
		$style= '';
		if(shu_fnc_theme_options('wc_show_background_related')){
			$style = 'style="background-image: url('.esc_url_raw( shu_fnc_theme_options('wc_show_background_related') ).');"';
		}
		$_id = rand();
			...
			<div class="widget products-collection owl-carousel-play woocommerce" id="postcarousel-<?php echo esc_attr($_id); ?>" data-ride="carousel">
				<div class="background" <?php echo trim($style); ?>>
					...
					<h3 class="widget-title">
				        <span><?php esc_html_e( 'Related Products', 'shu' ); ?></span>
					</h3>
