...
$readmore_text = !empty($readmore_text)? $readmore_text : esc_html__('Read more', 'shu');
				...
				<div class="blog-post-wrap">
					...
					<?php
						if ( has_post_format( array( 'chat', 'status' ) ) )
							$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'shu' );
						else
							$format_prefix = '%2$s';
						...
						if ( 'post' == get_post_type() && $show_author == 'yes' ) {
							printf( '<span class="author vcard"><i class="sl-user"></i> <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
								...
								esc_attr( sprintf( esc_html__( 'View all posts by %s', 'shu' ), get_the_author() ) ),
								get_the_author()
							);
						}
						...
						$tag_list = get_the_tag_list( '', esc_html__( ', ', 'shu' ) );
					...
					<?php if(!empty($show_button) && 'yes' === $show_button): ?>
						<a class="read-more" href="<?php echo esc_attr( get_permalink( $post->ID ) ); ?>"><?php echo trim( $readmore_text); ?></a>
					<?php endif; ?>					
				</div>
			</div>
