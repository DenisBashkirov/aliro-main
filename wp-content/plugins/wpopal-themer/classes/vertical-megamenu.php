...
class Wpopal_Themer_Megamenu_Vertical extends Walker_Nav_Menu {
   ...
   /**
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( isset($item->text_customize) && !empty($item->text_customize) ){
            switch ( $item->text_customize ) {
                case 'text_new':
                    $text_customize  = esc_html__('New', 'wpopal-themer');
                    break;

                case 'text_hot':
                    $text_customize  = esc_html__('Hot', 'wpopal-themer');
                    break;

                case 'text_featured':
                    $text_customize  = esc_html__('Featured', 'wpopal-themer');
                    break;

                case 'text_sale':
                    $text_customize  = esc_html__('Sale', 'wpopal-themer');
                    break;
    ...
    public function genMegaMenuByConfig( $item, $depth ){
        ...
        $post = get_post( $item->megamenu_profile ); $megamenu = 'true';
        if(is_object( $post)){
